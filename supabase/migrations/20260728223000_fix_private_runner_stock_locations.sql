-- Private runner-owned stock location remarks.
-- This table stores display-only notes and does not participate in stock calculations.
...
REVOKE ALL ON public.runner_stock_locations FROM anon;
GRANT SELECT, INSERT, UPDATE ON public.runner_stock_locations TO authenticated;

COMMENT ON TABLE public.runner_stock_locations IS
  'Private runner-owned display notes for locating stock; excluded from inventory balance calculations.';
