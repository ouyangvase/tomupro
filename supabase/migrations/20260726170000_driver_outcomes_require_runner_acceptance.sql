-- Driver performance outcomes only become final after Runner acceptance.
...
COMMENT ON FUNCTION public.get_driver_assignment_source(uuid, uuid, date, date, boolean, boolean) IS
  'Canonical driver assignments. Driver outcomes require Runner acceptance; direct Runner outcomes are inactive.';

CREATE OR REPLACE VIEW public.driver_monthly_ranking
WITH (security_invoker = true) AS
SELECT
  o.driver_id,
  driver_profile.display_name AS driver_name,
  driver_profile.avatar_url AS driver_avatar_url,
  rd.runner_id,
  runner_profile.display_name AS runner_name,
  date_trunc(
    'month',
    COALESCE(
      o.delivered_at,
  ...
  COUNT(*) FILTER (
    WHERE o.driver_status::text = 'DRIVER_FAILED'
      AND o.runner_status::text = 'FAILED_DELIVERY'
      AND (
        o.runner_accept_status::text = 'ACCEPTED'
        OR (
  ...
  RANK() OVER (
    PARTITION BY rd.runner_id, date_trunc(
      'month',
      COALESCE(
        o.delivered_at,
        public.order_operational_date(
...
JOIN public.runner_drivers rd
  ON rd.driver_id = o.driver_id
  AND rd.is_active = true
LEFT JOIN public.profiles driver_profile ON driver_profile.id = o.driver_id
LEFT JOIN public.profiles runner_profile ON runner_profile.id = rd.runner_id
WHERE o.driver_id IS NOT NULL
  AND (
    o.driver_id = auth.uid()
    OR rd.runner_id = auth.uid()
