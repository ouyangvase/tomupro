-- Fix column references in repair_missing_stock_deductions function
-- The function was using p.owner_id and w.owner_id but the correct columns are owner_user_id
...
BEGIN
  ...
  ELSE
    ...
    -- Get total missing count for comparison
    SELECT COUNT(DISTINCT oi.id) INTO v_missing_count
    FROM orders o
    JOIN order_items oi ON oi.order_id = o.id
    JOIN products p ON p.id = oi.product_id
