-- Fix: Add total_amount to the SELECT statement in process_delivery_queue_item
-- This fixes the error: record "v_order" has no field "total_amount"
...
BEGIN
  -- Get the order details (FIX: include total_amount for claim creation)
...
-- Reset all FAILED queue items with the total_amount error to PENDING for reprocessing
UPDATE delivery_queue
SET status = 'PENDING',
    error_message = NULL,
    retry_count = 0,
