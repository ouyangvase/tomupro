-- Update the reopen_rescheduled_orders function to auto-assign to same runner
...
BEGIN
  ...
  FOR order_record IN 
    ...
    WHERE o.next_delivery_date <= CURRENT_DATE
      ...
      AND o.runner_status NOT IN ('DELIVERED', 'CANCELLED')
  ...
  LOOP
    ...
    -- Reopen the order: move to READY status and keep same runner
    UPDATE orders
    SET 
      status = 'READY',
