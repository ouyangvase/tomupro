-- Keep driver-reported outcomes editable until the Runner accepts or reviews them.
-- This changes assignment visibility only; it does not create inventory movements.
...
COMMENT ON FUNCTION public.get_driver_assignment_source(uuid, uuid, date, date, boolean, boolean) IS
  'Canonical driver assignments. Unreviewed Driver outcomes remain pending and editable until Runner acceptance.';
