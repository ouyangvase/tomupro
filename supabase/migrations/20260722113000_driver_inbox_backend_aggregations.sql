-- Backend source of truth for the area-based Driver Inbox.
-- Collection amount is the COD cash amount the driver must collect:
-- orders.total_amount only when orders.payment_method = 'COD'; transfer orders collect 0.

CREATE OR REPLACE FUNCTION public.order_collection_amount(p_payment_method text, p_total_amount numeric)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
SET search_path = public
AS $$
  SELECT CASE WHEN upper(COALESCE(p_payment_method, '')) = 'COD' THEN COALESCE(p_total_amount, 0) ELSE 0 END
$$;

COMMENT ON FUNCTION public.order_collection_amount(text, numeric)
IS 'Returns driver collection amount. TOMUPRO stores COD amount in orders.total_amount when payment_method is COD; transfer orders require no driver cash collection.';

CREATE OR REPLACE FUNCTION public.order_operational_date(
...
SET search_path = public
AS $$
  SELECT COALESCE(p_next_delivery_date, p_expected_pickup_date, p_order_date)
$$;
...
AS $$
  WITH scoped AS (
    ...
    WHERE public.order_operational_date(o.next_delivery_date, o.expected_pickup_date, o.order_date) = p_operational_date
      AND (
...
BEGIN
  ...
  IF v_order_count <> array_length(p_order_ids, 1) THEN
    RAISE EXCEPTION 'Some selected orders were not found or are not on this operational date';
  END IF;
  ...
  VALUES (
    p_operational_date, 'NOTIFY_DRIVER', v_order_count, v_collect_amount, v_user, jsonb_build_object('status', 'queued')
  )
...
BEGIN
  ...
  VALUES (
    p_operational_date, 'SEND_TO_NEEDS_REVIEW', v_order_count, v_collect_amount, v_user, jsonb_build_object('status', 'applied')
  )
  RETURNING id INTO v_batch_id;
