-- Update the reopen_rescheduled_orders function to handle all valid runner statuses
-- Including TAKEN status which means runner already accepted the order
...
BEGIN
  ...
  -- Now includes orders with TAKEN runner_status (runner already accepted)
  FOR order_record IN 
    SELECT 
      o.id,
      o.order_code,
    ...
    WHERE o.next_delivery_date <= CURRENT_DATE
      ...
      AND o.runner_status NOT IN ('DELIVERED', 'CANCELLED', 'RETURNED_TO_OWNER')
