-- Fix: reference_id is UUID, not text - remove ::text casts
...
BEGIN
  ...
  -- Count missing deductions (UUID to UUID comparison)
  ...
  IF NOT p_dry_run AND v_missing_count > 0 THEN
    ...
    to_insert AS (
      SELECT DISTINCT ON (di.warehouse_id, di.product_id, di.order_item_id)
        ...
        di.order_item_id as reference_id,
    ...
    FROM orders o
    INNER JOIN order_items oi ON oi.order_id = o.id
    INNER JOIN stock_movements sm ON sm.reference_id = oi.id
  ...
  IF NOT p_dry_run THEN
    ...
    WITH delivered_items AS (
      ...
      WHERE o.runner_status = 'DELIVERED'
        ...
        AND NOT EXISTS (
          SELECT 1 FROM stock_movements sm
          WHERE sm.reference_id = oi.id AND sm.movement_type = 'SALE_DEDUCT'
        )
    ),
    to_insert AS (
      SELECT DISTINCT ON (di.warehouse_id, di.product_id, di.order_item_id)
...
-- Fix the unique index to work with UUID reference_id
