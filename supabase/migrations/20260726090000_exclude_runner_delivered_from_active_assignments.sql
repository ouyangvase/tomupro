-- Runner-completed orders can retain a stale driver_status from legacy flows.
-- They are historical records, not active driver work or pickup demand.
...
COMMENT ON FUNCTION public.get_driver_assignment_source(uuid, uuid, date, date, boolean, boolean) IS
  'Canonical assignment source. Active queries retain unfinished backlog but exclude runner-delivered history with stale driver status.';
