-- A driver delivery is only complete after the runner accepts it.
-- Before acceptance it is historical/inactive, not pending driver work.
...
COMMENT ON FUNCTION public.get_driver_assignment_source(uuid, uuid, date, date, boolean, boolean) IS
  'Canonical assignment source. Only runner-accepted deliveries are delivered; unaccepted driver deliveries are inactive.';
