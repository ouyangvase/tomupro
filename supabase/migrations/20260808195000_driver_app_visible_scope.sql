-- Use one canonical Driver app scope for Dashboard, Inbox and Excel.
-- Active work and unreviewed Driver outcomes remain visible. Final Runner
-- decisions and action-required/cancelled/returned work stay out of the app.
...
AS $$
  ...
  WHERE o.driver_id IS NOT NULL
    AND COALESCE(o.status::text, '') NOT IN (
...
COMMENT ON FUNCTION public.get_driver_assignment_source(uuid, uuid, date, date, boolean, boolean) IS
  'Canonical Driver app source: active work plus unreviewed Driver outcomes; final Runner outcomes are excluded.';
