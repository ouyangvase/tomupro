-- Keep the Driver App limited to active delivery work.
-- Driver-submitted Delivered/Failed rows remain available to Runner review,
-- but are never classified as active Driver assignments.
...
COMMENT ON FUNCTION public.get_driver_assignment_source(uuid, uuid, date, date, boolean, boolean) IS
  'Canonical Driver source: Driver App receives ACTIVE work only; submitted outcomes remain Runner-review data.';
