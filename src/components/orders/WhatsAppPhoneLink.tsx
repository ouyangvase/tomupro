...
// Minimal order type for the WhatsApp component - accepts both Order and query results
type OrderLike = {
  phone?: string | null;
  ...
  order_items?: Array<{
    qty?: number;
    product?: { sku_code?: string | null; sku_name?: string } | null;
...
/**
 * WhatsApp clickable phone link component
  ...
  return (
    <div className={`flex items-center gap-1.5 ${className}`}>
      <Tooltip>
        ...
        <TooltipContent>
          <p>Click to open WhatsApp chat</p>
        </TooltipContent>
      </Tooltip>
...
export function WhatsAppPhoneLinkCompact({ order, className = '' }: Omit<WhatsAppPhoneLinkProps, 'showIcon'>) {
  ...
  if (!isValid || !whatsappUrl) {
    return <span className={`text-sm ${className}`}>{phone}</span>;
  }
