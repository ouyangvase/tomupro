(function () {
  ...
  function buildShell() {
    ...
    shell.innerHTML = buildNav() + buildHero() + buildServiceCards() + buildAbout() + buildServices() + buildDashboard() + buildCod() + buildFeatures() + buildAreas() + buildTrack() + buildSuccess() + buildCta() + buildContact() + buildFooter() + buildAuthModal();
  ...
  function buildAbout() {
    return [
      '<section id="about" class="about-sec"><div class="wrap about-copy">',
      '<div class="eyebrow">About TOMUPRO</div>',
      '<h2>What TOMUPRO Does</h2>',
      '<p>TOMUPRO is a Brunei-based logistics and delivery operations platform operated by Tomu Enterprise, a registered sole proprietorship in Brunei Darussalam under Business Registration No. P30014276.</p>',
      '<p>TOMUPRO helps businesses manage last-mile delivery, dispatch, driver operations, pickup scheduling, order tracking, COD collection, fulfillment workflows, and delivery performance from one platform.</p>',
      '</div></section>'
    ].join("");
  }
